...
const Incomes = () => {
  ...
  // ── Shared modal form (reused in both desktop and mobile) ──
  const modalForm = (
    <BottomModal isOpen={showForm} onClose={resetForm} title={editingId ? "Modifier le revenu" : "Ajouter un revenu"}>
          ...
          <main className="desktop-main">
            ...
            <div className="desktop-greeting-toprow">
              <div className="desktop-greeting">
                <h1>Revenus 💰</h1>
                <p>Gérez vos sources de revenus pour ce mois.</p>
              </div>
            ...
            {loading && !showForm ? <LoadingSpinner /> : (
              <div className="desktop-main-grid">
                ...
                <div className="desktop-budget-card" style={{ overflowY: 'auto', maxHeight: 600 }}>
                  <p className="desktop-card-title">Historique des revenus</p>
                  {incomes.length === 0 ? (
                    <p style={{ color: '#B0B8C9', fontWeight: 600, textAlign: 'center', padding: '40px 0' }}>Aucun revenu ce mois.</p>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {incomes.map((inc, i) => <IncomeItem key={inc.id} inc={inc} i={i} />)}
                    </div>
                  )}
                </div>
              </div>
